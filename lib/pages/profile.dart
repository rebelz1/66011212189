...
import 'package:teste_mulator/model/res/get_custumer_res.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  late final GetCustumerRes getCustumerRes;
        ...
        builder: (context, snapshot) {
          ...
          return SingleChildScrollView(
            child: SizedBox(
              ...
              child: Column(
                children: [
                  // ✅ โหลดรูปจาก URL และ fallback ไป assets ถ้า error
                  Padding(
                    padding: const EdgeInsets.only(top: 16.0, bottom: 8),
                    child: Image.network(
                      getCustumerRes.image,
                      width: 150,
  ...
  if (res.statusCode == 200) {
    ...
    try {
      var res = await http.put(Uri.parse('$url/customers/${widget.cid}'),
  ...
  Future<void> loadDataAsync() async {
    ...
    var cidd = widget.cid;
    log(cidd.toString());
    var res = await http.get(Uri.parse('$url/customers/$cidd'));
    getCustumerRes = getCustumerResFromJson(res.body);
     log(getCustumerRes.toString());
    setState(() {
